...
#include "David's Vision Functions.h"
...
int main(int argc, char* argv[]){
	...
	// Constants
	const int SKIP = 2;						// Number of frames skipped after every retrieved and processed frame
	...
	while (true){
		...
		if (frameCount % (SKIP + 1) == 0){		// Allows us to skip frames
			...
			for (RotatedRect currentObject : rotatedRects){
				...
				// Calculate object's average hue
				if (pixCount != 0){
				...
				if (avgHue > 21 && avgHue < 39) {		// Delorean Identified
					...
					float orientation = currentObject.angle - 90;
